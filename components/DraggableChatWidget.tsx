...
export default function DraggableChatWidget() {
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      const rightPosition = Math.max(20, window.innerWidth - 120);
      const bottomPosition = Math.max(20, window.innerHeight - 120);
  ...
  // Debug logging
  console.log('DraggableChatWidget - User data:', user);
  console.log('DraggableChatWidget - User role:', user?.[0]?.role);
  console.log('DraggableChatWidget - Component rendering...');
  
  if (user?.[0]?.role !== 'admin') {
    console.log('DraggableChatWidget - Not admin, returning null');
    return null;
  }

  console.log('DraggableChatWidget - Admin confirmed, showing widget');

  // Simple test - always show a basic button first
  if (true) {
    return (
      <div 
      ...
      >
        <button
          ...
          className="bg-red-600 hover:bg-red-700 text-white p-4 rounded-2xl shadow-2xl border-2 border-red-500 flex items-center justify-center"
        >
    ...
    return (
      ...
      >
        <button
          ...
          className="chat-widget-button relative bg-blue-600 hover:bg-blue-700 text-white p-4 rounded-2xl shadow-2xl border-2 border-blue-500 flex items-center justify-center transition-all duration-300 transform hover:scale-105 group"
        >
  ...
  return (
    ...
    >
      <div className={`bg-white rounded-3xl shadow-2xl border border-gray-100 overflow-hidden transition-all duration-300 ${
        ...
        {/* Header */}
        <div 
          className="chat-header p-4 bg-gradient-to-r from-blue-600 to-blue-700 text-white cursor-move"
          onMouseDown={handleMouseDown}
        >
            ...
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-xs text-blue-100">
                  {currentChat ? 'Chatting with customer' : 'Select a conversation'}
                </p>
              </div>
            </div>
