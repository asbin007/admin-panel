...
function startServer() {
  try {
    ...
    io.on("connection", async (socket) => {
      ...
      if (token) {
        try {
          const result = jwt.verify(token as string, envConfig.jwtSecret as string) as any;
      ...
      // Handle order status updates - REMOVED USER ONLINE CHECK
      socket.on("updateOrderStatus", async (data) => {
        ...
        try {
          ...
          socket.emit("statusUpdated", { orderId, status });
      ...
      // Handle payment status updates - REMOVED USER ONLINE CHECK
